    ...
    <div class="px-8 pt-4 flex justify-center min-h-full">
        <div class="rounded-3xl bg-white shadow-2xl w-full max-w-6xl border border-gray-200">
            <div class="p-8">
                ...
                <p v-if="Object.keys(leveranciers_per_plaats).length !== 0" class="text-sm text-gray-400 mb-6">
                    Deze grafiek toont de verdeling van leveranciers over verschillende steden.
                </p>

                <!-- Fallback if no data -->
                <div v-if="Object.keys(leveranciers_per_plaats).length === 0" class="text-center text-gray-500 p-6">
                ...
                <!-- Chart -->
                <canvas v-else id="leveranciersPerPlaatsChart" class="max-h-screen w-full"></canvas>
            </div>
        </div>
    </div>
</template>
